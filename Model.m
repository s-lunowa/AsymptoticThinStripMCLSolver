...
%
%     p_in(t) - q(t) J(gamma(t)) == cos(theta(gamma(t), q(t)/w(gamma(t)))) / (Ca w(gamma(t)))
...
% To view a copy of this license, visit http://creativecommons.org/licenses/by/4.0/.
    ...
    methods
        ...
        function m = solveDAE(m, p_in, T, gamma0, options)
            ...
            options = odeset(options, 'Jacobian', {[],[0 0; 1 0]}, 'Events', @Model.stopAtEnds);

            % create model
            int_fun = @(x) 3.0 ./ (m.w(x) .* m.w(x) .* (m.w(x) + 3 * m.slip));
            integral01 = integral(int_fun, 0, 1);
            dae = @(t, y, dydt) ...
                [p_in(t) - y(2) .* (integral01 + (m.M - 1) * integral(int_fun, y(1),1)) ...
                    - cos(m.theta(y(1), y(2) ./ m.w(y(1)))) ./ (m.Ca * m.w(y(1))); ...
                 dydt(1) - y(2) ./ m.w(y(1))];
            % initial total flux (approx)
            tmp = integral01 + (m.M - 1) * integral(int_fun, gamma0,1);
            tmp_fun = @(q) p_in(0) - q .* tmp - cos(m.theta(gamma0, q ./ m.w(gamma0))) ./ (m.Ca * m.w(gamma0));
            q0 = fzero(tmp_fun, 0);
        ...
        function m = solveODE(m, q, T, gamma0, options)
            ...
            options = odeset(options, 'Events', @Model.stopAtEnds);
    ...
    methods (Access = protected)
        function [sat, p_cloc, tau] = computeSatP_clocTau(m, gamma, q)
            % COMPUTESATP_CLOCTAU computes the saturation, the local capillary pressure and the dynamic coefficient from a solution gamma, q.
            %
    ...
    methods (Static, Access = protected)
        function [value, isterminal, direction] = stopAtEnds(~,y,~)
            ...
            %
            % This function is used for odeXX in solveODE and solveDAE.
